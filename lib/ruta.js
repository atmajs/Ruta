(function(root, factory){
	...
	(function () {
		...
		route_parseDefinition = function route_parseDefinition(route, definition) {
			...
			for (; i < imax; i++) {
				...
				if (index !== 0) x = x.substring(index);
		...
		route_parsePath = function route_parsePath(route, path) {
	
			var queryIndex = path.indexOf('?'),
			    query = queryIndex === -1 ? null : path.substring(queryIndex + 1),
			    current = {
				path: path,
				params: query == null ? {} : query_deserialize(query, '&')
			};
	...
	// end:source parse.js
	// source match.js
	var route_match, route_matchAll, route_isMatch;
	
	(function () {
	
		route_match = function route_match(url, routes, method) {
		...
		route_matchAll = function route_matchAll(url, routes, method) {
			var parts = parts_deserialize(url),
			    imax = routes.length,
			    i = -1,
			    out = [];
		...
		route_isMatch = function route_isMatch(parts, route, currentMethod) {
	
			if (currentMethod != null && route.method != null && route.method !== currentMethod) {
				return false;
			}
	
			if (route.match) {
	
				return route.match.test(typeof parts === 'string' ? parts : parts_serialize(parts));
			}
	
			if (typeof parts === 'string') parts = parts_deserialize(parts);
			...
			if (route.query) {
				...
				for (key in route.query) {
					...
					if (c === ':') {
						...
						var alias = rgx_parsePartWithRegExpAlias(key);
						if (alias == null || hasKey(query, alias.matcher) === false) return false;
					...
					if (typeof value === 'string') {
						...
						if (query[key] == null) return false;
			...
			if (routeLength === 0) {
				if (route.strict) return parts.path.length === 0;
			...
			for (var i = 0, x, imax = arr.length; i < imax; i++) {
				...
				if (i >= routeLength) return route.strict !== true;
				...
				if (x.matcher) {
					if (x.matcher.test(arr[i]) === false) return false;
			...
			if (i < routeLength) return routePath[i].optional === true;
	...
	var Route = function Route(definition, value) {
	...
	;
		// end:source ./Route.es6
		// source ./RouteCollection.es6
		"use strict";
	...
	var RouteCollection = function () {
		...
		_createClass(RouteCollection, [{
			...
			key: "unshift",
			value: function unshift(def, value) {
				var route = new Route(def, value);
			...
			key: "remove",
			value: function remove(def, value) {
			...
			key: "get",
			value: function get(path, method) {
				return route_match(path, this.routes, method);
			}
		}, {
			key: "getAll",
			value: function getAll(path, method) {
			...
			key: "parse",
			value: function parse(definition, path) {
				var route = {};
	...
	;
		// end:source ./RouteCollection.es6
	
		Routes = RouteCollection;
	}());
	// end:source ../src/route/exports.js

	// source ../src/emit/exports.js
	var Location;
	(function(){
	...
	var HashEmitter = function () {
		...
		_createClass(HashEmitter, [{
			...
			key: 'navigate',
			value: function navigate(hash, opts) {
				this.opts = opts;
			...
			value: function supports() {
				if (typeof window === 'undefined' || 'onhashchange' in window === false) return false;
	...
	var HistoryEmitter = function () {
		...
		_createClass(HistoryEmitter, [{
			...
			value: function navigate(mix, opts) {
				...
				var isQueryObject = (typeof mix === 'undefined' ? 'undefined' : _typeof(mix)) === 'object',
				    url = null;
	...
	var LocationEmitter = function () {
		function LocationEmitter(collection, type) {
			_classCallCheck(this, LocationEmitter);
	
			this.collection = collection || new Routes();
			this.listeners = new Routes();
	
			if (type) {
				var Constructor = type === 'hash' ? HashEmitter : HistoryEmitter;
				this.emitter = new Constructor(this);
			}
	
			if (this.emitter == null && HistoryEmitter.supports()) this.emitter = new HistoryEmitter(this);
	
			if (this.emitter == null && HashEmitter.supports()) this.emitter = new HashEmitter(this);
		...
		_createClass(LocationEmitter, [{
			key: 'changed',
			value: function changed(path, opts) {
			...
			key: 'action',
			value: function action(route, opts) {
			...
			key: 'on',
			value: function on(def, cb) {
				this.listeners.push(def, cb);
			}
		}, {
			key: 'off',
			value: function off(def, cb) {
	...
	;
		// end:source LocationEmitter.es6
	
		Location = LocationEmitter;
	}());
	// end:source ../src/emit/exports.js
